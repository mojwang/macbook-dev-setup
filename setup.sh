...
main() {
    ...
    if [[ "$DRY_RUN" == true ]]; then
        ...
        echo ""
        echo "  1. ${BLUE}Recommended:${NC} Close this terminal and open a new one"
        echo "  2. ${BLUE}Quick reload:${NC} Run this command:"
        echo "     ${GREEN}source ~/.zshrc${NC}"
        echo "  3. ${BLUE}Full reload:${NC} Run this command:"
        echo "     ${GREEN}exec zsh${NC}"
        echo ""
        echo "Other next steps:"
        echo "• Review docs/manual-setup.md for additional configuration"
        echo "• Configure Claude CLI: ${GREEN}claude setup-token${NC}"
        echo ""
        echo "Happy coding!"
    fi
